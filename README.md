...
A small static web app for studying and testing common **English, Polish, and German** lemmas. You pick a prompt language and an answer language, type a translation, and get immediate correct / incorrect feedback with the expected dictionary form.

One lemma per word — nominative singular nouns, base adjectives, infinitive verbs. No extra declensions, conjugations, or alternate forms.
...
- Case-insensitive matching with trimmed whitespace. Diacritics are required. An on-screen **ą ć ę ł ń ó ś ź ż** bar appears when the answer language is Polish; **ä ö ü ß** appears when it is German.
...
```bash
npm test          # unit tests for answer matching and pair selection
...
`public/data/words.json` — a multilingual deck. Each entry is a concept with translations for the languages it covers (`en`, `pl`, `de`) and optional per-language frequency ranks.
